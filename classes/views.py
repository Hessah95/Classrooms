...
def classroom_create(request):
    if request.user.is_anonymous: #did not sign in
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            saveclassroom = form.save(commit=False)
            saveclassroom.teacher = request.user
            saveclassroom.save()
...
def student_create(request, classroom_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            savestudent = form.save(commit=False)
            savestudent.classroom = classroom
            savestudent.save()
            messages.success(request, "Successfully Created!")
